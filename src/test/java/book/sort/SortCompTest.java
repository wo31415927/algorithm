...
public class SortCompTest {
  ...
  protected QuickNonRecursionSampleSort quickNonRecursionSampleSort =
      new QuickNonRecursionSampleSort();
  ...
  protected HeapArraySort heapArraySort = new HeapArraySort();
  protected HeapLinkedSort heapLinkedSort = new HeapLinkedSort();
...
//    Integer[] arr = initArr(10000000, 0.00001);
  ...
  public void comp() throws Exception {
    for (int i = 0; i < sortList.size(); i++) {
      ...
      System.out.println(sortList.get(i).name() + "耗时(ms)：" + (System.currentTimeMillis() - from));
    }
  }
}
