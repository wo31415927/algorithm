...
public class Inversions {
     ...
     *
     * @param a the array
     * @return the number of inversions in the array. An inversion is a pair of indicies {@code i} and {@code j} such that {@code i < j} and {@code
     * a[i] > a[j]}.
     */
    ...
    public static <Key extends Comparable<Key>> long merge(Key[] a, Key[] aux, int lo, int mid, int hi) {
        long inversions = 0;
        // copy to aux[]
        for (int k = lo; k <= hi; k++) {
            aux[k] = a[k];
